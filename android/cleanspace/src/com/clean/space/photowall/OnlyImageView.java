	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (event.getPointerCount() == 1) {
				...
				// 进行缩放倍数检查，最大只允许将图片放大6倍，最小可以缩小到0.8初始化比例
				if ((currentStatus == Common.STATUS_OUT && totalRatio < 6.0f)
						|| (currentStatus == Common.STATUS_IN && totalRatio > 0.8f)) {
					scaledRatio = (float) (fingerDis / lastFingerDis);
					totalRatio = totalRatio * scaledRatio;
					if (totalRatio > 6.0f) {
						totalRatio = 6.0f;
					} else if (totalRatio < 0.8f) {
						totalRatio = 0.8f;
					}
					// 调用onDraw()方法绘制图片
					invalidate();
	...
	private void initBitmap(Canvas canvas) {
		if (sourceBitmap != null) {
			...
			if (bitmapWidth > width || bitmapHeight > height) {
				...
				float sw = (1.0f * width) / bitmapWidth;
				float sh = (1.0f * height) / bitmapHeight;
				totalRatio = initRatio = sw < sh ? sw : sh;
				totalRatio = initRatio = initRatio > 6.0f ? 6.0f : initRatio;
				float translateX = (width - bitmapWidth * initRatio) / 2f;
				float translateY = (height - bitmapHeight * initRatio) / 2f;
//				translateX = translateX / initRatio;
//				translateY = translateY / initRatio;
				totalTranslateX = translateX;
				totalTranslateY = translateY;
